...
for y in {1982..2000}; do
    if [ $USCOUNTER -eq $SLURM_ARRAY_TASK_ID ]; then
        echo "synchronic: year $y in: $arch1 vs. $arch2"
        python words_are_malleable4.py --path1 $EMB_DIR1 --path2 $EMB_DIR2 --path3 $EMB_DIR3 --model1 ${y}.bin --model2 ${y}.bin --model3 ${y}.bin --model1_name ${arch1}_${y} --model2_name ${arch2}_${y} --model3_name ${arch3}_${y} --k ${k} --words_file ${keywords_file} --method=combined --save_dir ${savedir}
    fi
