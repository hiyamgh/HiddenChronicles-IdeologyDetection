    ...
    for k in ${neighs[@]}; do
        for y in {1983..2009}; do
            ...
            if [ $USCOUNTER -eq $SLURM_ARRAY_TASK_ID ]; then
                python words_are_malleable.py --path1 $EMB_DIR1 --path2=$EMB_DIR1 --model1 ${yprev}.bin --model2 ${ycurr}.bin --model1_name ${arch1}_${yprev} --model2_name ${arch2}_${ycurr} --k ${k} --t ${t} --words_file ${keywords_file} --method=linear
            fi
