...
def align_years(years, rep_type, in_dir, out_dir, count_dir, min_count, **rep_args):
    ...
    for year in years:
        ...
        path = in_dir + 'word2vec_{}'.format(str(year))
        # year_embed =  create_representation(rep_type,path, **rep_args)
        year_embed = Word2Vec.load(path)
        ...
        if first_iter:
            ...
            # if year == 1935:
            #     print()
            aligned_embed = alignment.smart_procrustes_align_gensim(base_embed, year_embed)
        base_embed = aligned_embed
        print("Writing year:", year)
        foutname = out_dir + str(year)
        # np.save(foutname + "-w.npy", aligned_embed.m)
        np.save(foutname + "-w.npy", aligned_embed.wv.vectors) # wv.vectors
        # write_pickle(aligned_embed.iw, foutname + "-vocab.pkl")
        write_pickle(aligned_embed.wv.index2word, foutname + "-vocab.pkl")
...
if __name__ == "__main__":
    ...
    end_year = 2005
    year_inc = 1
    dir = 'F:/newspapers/word2vec/nahar/embeddings/'
    years = range(start_year, end_year + 1, year_inc)

    # out_dir = args.dir + "/aligned/"
    out_dir = 'D:/aligned/'
    mkdir(out_dir)
    align_years(years, "", dir, out_dir, "", "")
