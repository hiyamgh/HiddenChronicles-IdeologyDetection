...
if __name__ == '__main__':
    ...
    subdir = "../data/validation/validation/"
    files = os.listdir(subdir)
    for file in files:
        if '.txt' in file:
    ...
    sentences, labels, labels_general, speeches = [], [], [], []
    for doc in train_data:
        for sent_id, sent in doc.sentences.items():
            ...
            if sent_id in doc.sent_to_speech:
                speeches.append('Speech')
            else:
                speeches.append('Not Speech')

    df = pd.DataFrame()
    df['Sentence'] = sentences
    df['Label'] = labels
    df['Label_general'] = labels_general
    df['Speech_label'] = speeches
