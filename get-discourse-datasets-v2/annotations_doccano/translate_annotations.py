...
if __name__ == '__main__':
    ...
    df = pd.read_excel('our_corpus.xlsx')
    translator = Translator()

    df_trans = pd.DataFrame(columns=['Original', 'Sentence'])
    df_to_save_name = 'our_corpus_{}.xlsx'.format(lang)
        ...
        if num_rows < len(df):
            ...
            for j, row in tqdm(df.iterrows(), total=df.shape[0], desc='Translating to {}'.format(codes2names[lang])):
                if j >= num_rows:
                    ...
                    if sentence != "":
                        try:
                            translated = translator.translate('{}'.format(sentence), src='ar', dest=lang).text
                        except:
                            try:
                                print('was not able to translate row {}: {}'.format(j, row['text']))
