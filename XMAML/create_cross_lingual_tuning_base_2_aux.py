...
if __name__ == '__main__':
    ...
    inner_train_steps_grid = [3, 5, 7, 10, 15]  # 7, 10, and 15 are causing CUDA out of memory errors
    ...
    lang_pairs = list(itertools.combinations(list(codes2names.keys()), 2))
    ...
    with open(os.path.join(save_dir, 'VDC_cross_lingual_base.txt'), 'w') as f:
        for lang_pair in lang_pairs:
            if 'ar' not in lang_pair:
                dev_dataset_ids = ['corp_PRST_{}_VDC'.format(lang_pair[0]), 'corp_PRST_{}_VDC'.format(lang_pair[1])]
                dev_dataset_fine_tune_id = ['corp_PRST_ar_VDC']
                test_dataset_id = ['corp_SSM_ar_VDC']
                                        ...
                                        for model in models:
                                            f.write(
                                                "--bert_model {} --model_type {} --dev_datasets_ids {} --dev_dataset_finetune {} --test_dataset_eval {} --do_validation 1 --do_finetuning {} --do_evaluation 1 --meta_learn_iter {} --n {} --per_gpu_train_batch_size {} --inner_train_steps {} --inner_lr {} --labels {}  --output_dir_meta {} --eval_task_dir {}\n".format(
                                                    ...
                                                    ",".join(all_labels_VDC),
                                                    "results_tuning_cross_lingual/VDC/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft),
                                                    "results_tuning_cross_lingual/VDC/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/eval/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft)
                                                ))
    ...
    with open(os.path.join(save_dir, 'VDS_cross_lingual_base.txt'), 'w') as f:
        for lang_pair in lang_pairs:
            if 'ar' not in lang_pair:
                dev_dataset_ids = ['corp_PRST_{}_VDS'.format(lang_pair[0]), 'corp_PRST_{}_VDS'.format(lang_pair[1])]
                dev_dataset_fine_tune_id = ['corp_PRST_ar_VDS']
                test_dataset_id = ['corp_SSM_ar_VDS']
                                        ...
                                        for model in models:
                                            f.write(
                                                "--bert_model {} --model_type {} --dev_datasets_ids {} --dev_dataset_finetune {} --test_dataset_eval {} --do_validation 1 --do_finetuning {} --do_evaluation 1 --meta_learn_iter {} --n {} --per_gpu_train_batch_size {} --inner_train_steps {} --inner_lr {} --labels {} --output_dir_meta {} --eval_task_dir {}\n".format(
                                                    ...
                                                    ft, m_iter, n, bs, inner_train_step, inner_lr,
                                                    ",".join(all_labels_VDS),
                                                    "results_tuning_cross_lingual/VDS/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft),
                                                    "results_tuning_cross_lingual/VDS/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/eval/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft)
                                                ))
                                            model_i += 1
    f.close()
    ...
    with open(os.path.join(save_dir, 'argumentation_cross_lingual_base.txt'), 'w') as f:
        for lang_pair in lang_pairs:
            if 'ar' not in lang_pair:
                dev_dataset_ids = ['corp_PRST_{}_ARG'.format(lang_pair[0]), 'corp_PRST_{}_ARG'.format(lang_pair[1])]
                dev_dataset_fine_tune_id = ['corp_PRST_ar_ARG']
                test_dataset_id = ['corp_SSM_ar_ARG']
                                        ...
                                        for model in models:
                                            f.write(
                                                "--bert_model {} --model_type {} --dev_datasets_ids {} --dev_dataset_finetune {} --test_dataset_eval {} --do_validation 1 --do_finetuning {} --do_evaluation 1 --meta_learn_iter {} --n {} --per_gpu_train_batch_size {} --inner_train_steps {} --inner_lr {} --labels {} --output_dir_meta {} --eval_task_dir {}\n".format(
                                                    ...
                                                    ft, m_iter, n, bs, inner_train_step, inner_lr,
                                                    ",".join(all_labels_ARG),
                                                    "results_tuning_cross_lingual/ARG/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft),
                                                    "results_tuning_cross_lingual/ARG/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/eval/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft)
                                                ))
                                            model_i += 1
    f.close()
    ...
    with open(os.path.join(save_dir, 'PTC_cross_lingual_base.txt'), 'w') as f:
        for lang_pair in lang_pairs:
            if 'ar' not in lang_pair:
                dev_dataset_ids = ['corp_PRST_{}_PTC'.format(lang_pair[0]), 'corp_PRST_{}_PTC'.format(lang_pair[1])]
                dev_dataset_fine_tune_id = ['corp_PRST_ar_PTC']
                test_dataset_id = ['corp_SSM_ar_PTC']
                                        ...
                                        for model in models:
                                            f.write(
                                                "--bert_model {} --model_type {} --dev_datasets_ids {} --dev_dataset_finetune {} --test_dataset_eval {} --do_validation 1 --do_finetuning {} --do_evaluation 1 --meta_learn_iter {} --n {} --per_gpu_train_batch_size {} --inner_train_steps {} --inner_lr {} --labels {} --output_dir_meta {} --eval_task_dir {}\n".format(
                                                    ...
                                                    ft, m_iter, n, bs, inner_train_step, inner_lr,
                                                    ";".join(all_labels_PTC),
                                                    "results_tuning_cross_lingual/PTC/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft),
                                                    "results_tuning_cross_lingual/PTC/{}-{}-{}-m_iter{}-inner_lr{}-inner_train_step{}-n{}-per_gpu_train_batch_size{}-ft{}/eval/".format(
                                                        lang_pair[0], lang_pair[1], model, m_iter, inner_lr, inner_train_step, n, bs, ft)
                                                ))
                                            model_i += 1
    f.close()