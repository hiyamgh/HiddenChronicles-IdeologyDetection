...
class DiscourseProfilingProcessorAR:
...
def main():
    ...
    processors = {
        # "ptc": PTCProcessor,
        "discourse_profiling": DiscourseProfilingProcessorAR,
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # save to output dir
        print('saving results to {} ....'.format(os.path.join(args.output_dir, 'test_actual_predicted.xlsx')))
        test_set = pd.read_excel(args.test_set)
        df = pd.DataFrame()
        df['actual'] = actual_strings
        df['predicted'] = preds_strings
        df['Sentence_ar'] = test_set['Sentence_ar']
        df['Year'] = test_set['Year']
        df.to_excel(os.path.join(args.output_dir, 'test_actual_predicted.xlsx'), index=False)
