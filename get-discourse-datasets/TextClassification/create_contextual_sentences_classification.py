...
parser.add_argument('--archive', type=str, default='assafir', help='name of the archive')
...
if __name__ == '__main__':
    ...
    for file in ['df_test_{}.xlsx'.format(y) for y in [1982, 1985, 1986, 2001, 2005, 2006, 2007]]:
        df = pd.read_excel(os.path.join(save_dir, file))
        print('{}: {}'.format(file, list(set(list(df['Keyword'])))))
        for keyword in mappingsar2en:
            df = df[df.Keyword == keyword]
            if len(df) > 1:
                df.to_excel(os.path.join(save_dir, '{}_{}.xlsx'.format(file[:-5], mappingsar2en[keyword])), index=False)
            else:
                print('word {} not found in {}'.format(keyword, file))