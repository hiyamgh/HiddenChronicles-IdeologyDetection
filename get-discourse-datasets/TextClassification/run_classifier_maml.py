...
class Webis16Processor:
    ...
    def get_next_batch(self, B, N, K, Q, set_type):
        print()


class DiscourseProfilingProcessor:
...
class AmazonProcessor(DataProcessor):
    ...
    def _divide_tasks(self,data_name, type):
        ...
        for task in task_class:
            ...
            print(file_name)
            task_data = self._read_file(file_name) # list of all sentences for any of: t2/t5/t4
            tasks.append(task_data)
            total_len = total_len+len(task_data)

        return tasks, total_len # tasks is a list of 3, each contains list of lines for t2/t4/t5,
        # total_len is the total number of lines in the Union of t2/t4/t5
    ...
    def _get_examples(self, data_dir, filter_name, type):
        ...
        for task_name in task_list:
            ...
            tasks.extend(diverse_task) # in workspace.filtered.list, there are 23, tasks, each has t2/t4/t5, so 23*3=69
                                        # in workspace.target.list, tehre are 4 tasks, each has t2/t4/t5, so 4*3=12
            total_len += task_len
        return tasks, total_len # tasks is a list of 69 or 12, each contains lines, tital_len is total number of lines
        # as a whole
    ...
    def load_all_data(self, data_dir):
        ...
        for id in range(self.train_task_num):
            examples, labels = self.get_train_examples(data_dir, id) # convert each line into InputExamples
            self.train_examples.append(examples)
            self.train_labels.append(labels)
            examples, labels = self.get_dev_examples(data_dir,id) # convert each line into InputExamples
            self.dev_examples.append(examples)
            self.dev_labels.append(labels)
            examples, labels = self.get_test_examples(data_dir, id) # convert each line into InputExamples
            self.test_examples.append(examples)
            self.test_labels.append(labels)
        for id in range(self.test_task_num):
            examples, labels = self.get_fsl_train_examples(data_dir, id) # convert each line into InputExamples
            self.fsl_train_examples.append(examples)
            self.fsl_train_labels.append(labels)
            examples, labels = self.get_fsl_dev_examples(data_dir, id) # convert each line into InputExamples
            self.fsl_dev_examples.append(examples)
            self.fsl_dev_labels.append(labels)
            examples, labels = self.get_fsl_test_examples(data_dir, id) # convert each line into InputExamples
    ...
    def get_next_batch(self, B, N, K, Q, set_type, task_id):
        ...
        query = []
        if set_type == 'train':
            train_examples = self.train_examples[task_id] # here we are getting train_examples of a particular
            # task id, however, the general aim is to get train_examples (in case of supervised_reptile)
            train_labels = self.train_labels[task_id]
            dev_examples = self.dev_examples[task_id]
            dev_labels = self.dev_labels[task_id]
        else:
            return support, query
        label_names = self.get_labels() # there are only two labels: [-1, 1]
        true_train_indices = [i for i, e in enumerate(train_labels) if e == label_names[0]] # all indices of class -1
        false_train_indices = [i for i, e in enumerate(train_labels) if e == label_names[1]] # all indices of class 1
        ...
        for one_sample in range(B):
            select_indices = np.random.choice(true_train_indices, K, False) # K-shot examples from class -1
            ...
            select_indices = np.random.choice(false_train_indices, K, False) # K-shot examples from class 1
            support.extend([train_examples[i] for i in select_indices])
...
def main():
    ...
    processor.load_all_data(args.data_dir) # train, dev, and test examples are saved as lists of InputExamples
    print("load finished!")
    ...
    if args.do_train:
        ...
        for epoch in trange(int(N_iteration), desc="Iterations"): # meta-training
            ...
            for task_id in tqdm(selected_tasks,desc="Task"): # for each task in randomly selected tasks -- this is the first loop over meta_batch_size
                ...
                task_acc = 0
                for _ in range(Inner_epochs): # for inner number of updates -- this is the inner loop over mini batches
                    ...
                    train_support, train_query = processor.get_next_batch(train_batch_s, N_class, N_shot, N_query, 'train', task_id) # sample support and query sets from the selected task
    ...
    # if transductive: we forward the test set (from outer loop) and calculate resuults
