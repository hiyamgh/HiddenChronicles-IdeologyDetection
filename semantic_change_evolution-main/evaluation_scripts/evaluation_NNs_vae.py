...
def data(model_name, TEST_ON):  # model_name:{r,f,rf}; TEST_ON: 1-13
    ...
    # with open('../results_final/results_seq2seq_' + model_name + '_vae/4model2.p', 'rb') as f:
    #     model = pickle.load(f)
    # import keras
    # model = keras.models.load_model('../results_final/results_seq2seq_' + model_name + '_vae/4model2.p', custom_objects={'tf': tf})
    return X, Y, model
...
def data_baselines(model_name, TEST_ON):  # reconstruct, future; 1-13
    ...
    X = np.concatenate(
        (ts[test_idx, 0, :].reshape(-1, 1, ts.shape[2]), ts[test_idx, TEST_ON, :].reshape(-1, 1, ts.shape[2])), axis=1)

    if model_name == 'r':
        Y = np.concatenate(
            (ts[test_idx, 0, :].reshape(-1, 1, ts.shape[2]), ts[test_idx, TEST_ON, :].reshape(-1, 1, ts.shape[2])),
            axis=1)
    elif model_name == 'f':
