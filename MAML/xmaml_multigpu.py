...
def x_maml_with_aux_langs(args, model, lang_datasets, tokenizer, suffix=None):
    ...
    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.gpus)
    ...
    if args.local_rank != -1:
        print('Initializing model DDP with local rank: {}'.format(args.local_rank))
...
def evaluate_task(args, model, eval_dataset, label_list, early=False, prefix=""):
    ...
    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.gpus)
...
def run_train(gpu, args):
    ...
    logger.warning("Process rank: %s, device: %s, n_gpu: %s, distributed training: %s, 16-bits training: %s",
                   args.local_rank, device, args.gpus, bool(args.local_rank != -1), args.fp16)
    ...
    # get list of datasets for xmaml
...
def main():
    ...
    ## DDP Training Parameters
    parser.add_argument("--nodes", type=int, default=1, help="number of nodes needed for DDP multi-node multi gpu training")
    parser.add_argument("--gpus", type=int, default=1, help="number of GPUs per node needed for DDP multi/single node multi-gpu training")
    parser.add_argument("--ip_address", type=str, default=None, help="ip address of master node -- needed for DDP single/multi-node multi-gpu training")
    parser.add_argument("--backend", type=str, default="gloo", help="backend needed for single/multi-node multi-gpu training")
    ...
    logger.info("Fine Tuning at rank {} ...".format(args.local_rank))
    # Fine tuning
    ...
    logger.info("Evaluating at rank: {}...".format(args.local_rank))
    # # Evaluation
    # model = BertForSequenceClassification.from_pretrained(args.output_dir_meta, num_labels=num_labels)
    # model = model.to(args.device)
